<?php
// config/db.php — generated by installer. Do not edit manually.

// ── Site base URL (change if installed in a different subfolder) ──────────────
require_once __DIR__ . '/../includes/cache.php';
// ── Dynamic BASE URL Detection ──────────────────────────────────────────────
if (!defined('BASE')) {
    $script = $_SERVER['SCRIPT_NAME'] ?? '';
    // Strip everything after the last known root file (like index.php or any direct root access)
    // fallback to parsing up to the expected directory name 'advet'
...
define('DB_NAME', 'advet');
define('DB_USER', 'advet');
define('DB_PASS', 'advet');
...
if (!function_exists('uploadSiteLogo')) {
    function uploadSiteLogo(array $file): string|false {
        ...
        if ($file['size'] > 2 * 1024 * 1024) return false; // 2MB max for logo

        $uploadDir = __DIR__ . '/../assets/uploads/site/';
        if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);

        $ext      = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
        $filename = 'logo_' . time() . '.' . $ext; // Reverted to original logic for filename
...
if (!function_exists('formatPrice')) {
    function formatPrice(float $price): string {
        ...
        if ($currency === 'INR') {
            $num = number_format($price, 0, '.', ''); // Get full number string without decimals
...
if (!function_exists('recordFlash')) {
    function recordFlash(string $type, string $msg): void {
