<?php
// FILE: admin/featured-commercial.php
session_start();
...
<head>
    ...
    <title>Property Categories | Advet Studio</title>
    ...
    <div class="max-w-6xl mx-auto">
        ...
        <header class="flex justify-between items-end mb-12 form-reveal">
            <div>
                ...
                <h1 class="text-4xl font-serif font-light italic">Add Commercial <span class="text-muted">Properties</span></h1>
            </div>
            <a href="<?= BASE ?>admin/add-featured-commercial.php" 
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 form-reveal" style="animation-delay: 0.1s">
            <?php if (empty($archetypes)): ?>
                <div class="col-span-full py-20 text-center bg-background rounded-[2.5rem] border border-sand/40">
                    <p class="text-serif italic text-muted text-xl">No Commercial Properties added yet</p>
                </div>
            <?php endif; ?>

            <?php foreach ($archetypes as $a): ?>
            <div class="bg-background rounded-[2.5rem] border border-sand/40 overflow-hidden shadow-sm group">
                <div class="aspect-[4/5] relative overflow-hidden bg-surface">
                    <?php if($a['image_path']): ?>
                        <img src="<?= e(imgUrl($a['image_path'])) ?>" class="w-full h-full object-cover transition-transform duration-[5s] group-hover:scale-105">
                    <?php else: ?>
                        <div class="w-full h-full flex items-center justify-center text-muted/30 italic text-xs">No image</div>
                    <?php endif; ?>
                </div>
                <div class="p-8">
                    <h3 class="text-lg font-serif mb-2"><?= e($a['title']) ?></h3>
                    <p class="text-xs text-muted font-light leading-relaxed mb-6 line-clamp-2"><?= e($a['description']) ?></p>
                    
                    <div class="flex items-center justify-between pt-6 border-t border-sand/30">
                        <a href="<?= BASE ?>admin/add-featured-commercial.php?id=<?= $a['id'] ?>" class="text-[10px] font-bold uppercase tracking-widest text-accent hover:text-accent-dark transition-colors">Edit</a>
                        <form method="POST" action="<?= BASE ?>actions/delete-commercial.php" onsubmit="return confirm('Remove this archetype from the gallery?');">
